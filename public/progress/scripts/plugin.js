(function ( $ ) {
    ...
    var methods = {
        init : function(options) {
            ...
            if(global_settings.starting_position != 20) {
                global_settings.starting_position = global_settings.starting_position % 20;
            }
            if(global_settings.ending_position != 20) {
                global_settings.ending_position = global_settings.ending_position % 20;
            }
            // No 'px' or '%', add 'px'
    ...
    function calcPos(apples, percent) {
        if(percent < 0) {
            ...
            var starting_degree = (parseInt(apples) / 20) * 360;
            var starting_radian = starting_degree * (Math.PI / 180);
            return starting_radian - (Math.PI / 2);
        } else {
            // Calculate ending position
            var ending_degree = ((parseInt(apples) + parseInt(percent)) / 20) * 360;
    ...
    function insertText(scope) {
        $(".progress-percentage").text(Math.round(global_settings.percent) + "/20");
    }
    // create canvas
